...
console.log(month)
```
10. вывод строки при помощи форматированния  ${}.
```javascript
...
console.log(num)
``` 
12.  filter  функция возвращает отфильтрованный массив(в задаче массив четных чисел).  reduce  функция идет по массиву и кладет значение в  total(суммируем все квадратные корни чисел). 
...
``` 
13.
 3.4499999999999997 - ответ из-за плавующей точки

```javascript
console.log(1.15 + 2.30)
``` 
14.
   5 - ответ  т.к. оператор ++ происходит в последнюю очередь.
```javascript
let x = 5
console.log(x++)
console.log(x)
